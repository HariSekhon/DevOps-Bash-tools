...
if ! docker ps -qf name="$container_name" | grep -q .; then
    timestamp 'booting MariaDB container:'
    docker run -d -ti \
               --name "$container_name" \
               -p 3306:3306 \
    ...
    while true; do
        ...
        sleep 1
        if [ $tries -gt 60 ]; then
            echo "MariaDB container failed to become ready for connections within reasonable time, check logs (format may have changed):"
            echo
