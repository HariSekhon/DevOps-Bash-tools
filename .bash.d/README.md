...
* `<technology>.sh` - aliases, functions and environment variables to make interactive day-to-day use of a specific technologies easier
  * Cloud / Containerization / Virtualization:
    * `aws.sh` - `aws_env` function populates [AWS](https://aws.amazon.com/) credentials from `~/.boto` / `~/.aws/credentials` to `$AWS_ACCESS_KEY` and `$AWS_SECRET_KEY` environment variables for use with other tools without having to maintain the credentials in multiple places. `awk_token` generates a 24-hour session token and loads to environment for use with [AWS CLI](https://aws.amazon.com/cli/), as well as saving to `~/.aws/token` for loading to other shells calling `aws_env`
