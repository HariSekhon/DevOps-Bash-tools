...
- `kafka_wrappers/*.sh` - scripts to make [Kafka](http://kafka.apache.org/) cli usage easier including auto-setting Kerberos to source TGT from environment and auto-populating broker and zookeeper addresses. For something similar for [Solr](https://lucene.apache.org/solr/), see `solr-cli.pl` in [DevOps Perl Tools](https://github.com/harisekhon/devops-perl-tools)
