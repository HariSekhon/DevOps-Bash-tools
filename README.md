...
- `impala_shell.sh` - connects to a random [Impala](https://impala.apache.org/) node via impala-shell, parsing the Hadoop topology map and selecting a random datanode to connect to Impalad. This is mostly for convenience to shorten commands and while it acts as a poor man's load balancer, you might want to instead use my real load balancer [HAProxy config for Impala](https://github.com/HariSekhon/HAProxy-configs) (and many Big Data & NoSQL technologies). Can specify explicit `$IMPALA_HOST` environment variable override and also enable SSL via `IMPALA_SSL=1` environment variable or regular impala-shell `--ssl` argument
...
- `perl*.sh` - various Perl utilities including scripts to generate fatpacks (self-contained perl scripts with all modules dependencies embedded in them), finding the Perl library base, finding where a Perl CLI tool is installed (system vs user, useful when it gets installed to a place that isn't in your `$PATH`, where `which` won't help), finding all perl paths that will be searched for cpan modules, finding unused cpan modules in project, finding duplicate cpan modules between projects and sub-projects, printing the perl module search path, bulk installing files lists of cpan modules, installing cpan packages only when not present in perl path via system packages to avoid needless installations, saving time and reducing build failures
- `python*.sh` - various Python utilities including scripts for pre-byte-compiling, finding the Python library base, finding where a Python CLI tool is installed (system vs user, useful when it gets installed to a place that isn't in your `$PATH`, where `which` won't help), finding all python paths that will be searched for modules, finding unused pip modules in projects, finding duplicate pip modules between projects and sub-projects, converting Python module names to import names, printing the python module search path, installing packages, installing packages only when not present in python path (useful for speeding up builds by skipping any pip modules that are supplied via already installed OS packages - all builds across all my github repos now `make system-packages` before `make pip` to shorten how many packages need installing, reducing chances of build failures)
