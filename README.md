...
- `mysqld.sh` - one-touch MySQL test environment, boots docker container + drop in to `mysql` shell, with `/sql` scripts mounted in container for easy sourcing eg. `source /sql/<name>.sql`
...
- `postgres.sh` - one-touch Postgres test environment, boots docker container + drops in to `psql` shell, with `/sql` scripts mounted in container for easy sourcing eg. `\i /sql/<name>.sql`
