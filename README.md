...
- `kubernetes_join_cmd.sh` - outputs `kubeadm join` command (calculates cert hash + new token) to join an existing Kubernetes cluster. Useful to simplify cli or scripting (used in [vagrant kubernetes](https://github.com/HariSekhon/DevOps-Bash-tools/tree/master/vagrant/kubernetes) provisioning scripts)
