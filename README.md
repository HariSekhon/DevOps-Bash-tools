...
- [AWS](https://aws.amazon.com/) scripts - `aws_*.sh`:
  ...
  - `aws_sso_config_duplicate_sections.sh` - finds duplicate AWS SSO config sections that are using the same sso_account_id
  - `aws_sso_config_duplicate_profile_names.sh` - finds duplicate AWS SSO config profile names that are using the same sso_account_id
