...
# shields.io gets throttled after a few reloads :-/ - which is bad because shields latency is high and oftens fails to load some badges which requires you to reload the page a few times until they load
...
- `concourse.sh` - launches [Concourse CI](https://concourse-ci.org/) in docker, configures pipeline using `$PWD/.concourse.yml`, initiates build and tails results in terminal. Instant Concourse CI at one touch, can be called from any repo's top level directory that has a `.concourse.yml` config (all mine have it), mimicking structure of most of fully managed CI systems also used in this and adjacent repos
