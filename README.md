...
- `mp3_set_track_order.sh` - sets the track order metadata for mp3 files under given directories to follow the lexical file naming order. Useful for correctly ordering albums songs and audiobook chapters eg. for Mac's Books.app. Especially useful when there are multiple CD sub-folders which you want to group into a single audiobook using `mp3_set_album.sh` and then fix the ordering to progress each CD's tracks in the correct sequence (otherwise they'd interleave in Mac's Books.app eg. prevents CD1 track1, CD2 track1, CD1 track2 ...)
