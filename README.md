...
- [Google Cloud](https://cloud.google.com/) scripts - `gcp_*.sh` / `gce_*.sh` / `gke_*.sh` / `gcr_*.sh` / `bigquery_*.sh`:
  ...
  - `gcp_sql_*.sh` - [Cloud SQL](https://cloud.google.com/sql) scripts:
    ...
    - `gcp_sql_proxy.sh` - boots a [Cloud SQL Proxy](https://cloud.google.com/sql/docs/postgres/sql-proxy) to all Cloud SQL instances in given or all GCP projects for fast convenient direct `psql` / `mysql` access via local sockets. Installs Cloud SQL Proxy if necessary
...
- `teamcity_*.sh` - [TeamCity CI](https://www.jetbrains.com/teamcity/) API scripts:
  ...
  - `teamcity_upload_ssh_key.sh` - uploads an SSH key to a TeamCity project (for use in VCS root connections)
