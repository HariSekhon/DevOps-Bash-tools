...
- `terraform_*.sh` - [Terraform](https://www.terraform.io/) scripts:
  ...
  - `terraform_registry_url_extract.sh` - extracts the Terraform Registry URL in either `tfr://` or `https://registry.terraform.io/` format from a given file or standard input. Useful to fast load Terraform Module documentation via editor/IDE hotkeys. Based on `urlextract.sh` above in which it is included
