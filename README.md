...
[![Lines of Code](https://img.shields.io/badge/lines%20of%20code-38k-lightgrey?logo=codecademy)](https://github.com/HariSekhon/DevOps-Bash-tools#hari-sekhon---devops-bash-tools)
...
350+ DevOps Shell Scripts and Advanced Bash environment.
...
- `gcp_info.sh` - huge [Google Cloud](https://cloud.google.com/) inventory of deployed resources within the current project - Cloud SDK info, Auth & Configurations, GCE Virtual Machine instances, Cloud SQL instances, App Engine instances, Cloud Functions, GKE clusters, Pods across all Kubernetes clusters, Services & APIs enabled, Service Accounts, Networks, Routes, VPNs, Firewall rules, Forwarding rules, Cloud Storage Buckets, Dataflow jobs, PubSub topics, BigTable clusters and instances, Datastore Indexes etc.
...
- `gce_*.sh` - [Google Cloud](https://cloud.google.com/) scripts for [GCE](https://cloud.google.com/compute/) metadata API and pre-emption
- `gke_kube_creds.sh` - [Google Cloud](https://cloud.google.com/) scripts for [GKE](https://cloud.google.com/kubernetes-engine) to auto-load all Kubernetes credentials and contexts for all clusters in the current project so your `kubectl` is ready to rock
...
- `gocd.sh` - one-touch [GoCD](https://www.gocd.org/), launches in docker, (re)creates config repo (`$PWD/setup/gocd_config_repo.json`) from which to source pipeline(s) (`.gocd.yml`), detects and enables agent(s) to start building. Call from any repo top level directory with a `.gocd.yml` config (all mine have it), mimicking structure of fully managed CI systems
