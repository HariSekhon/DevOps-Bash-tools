...
- `mysqld.sh` - one-shot docker container boot + drop in to `mysql` shell for quick testing, with `/sql` scripts mounted in container for easy sourcing eg. `source /sql/<name>.sql`
...
- `postgres.sh` - one-shot Postgres docker container boot + drop in to `psql` shell for quick testing, with `/sql` scripts mounted in container for easy sourcing eg. `\i /sql/<name>.sql`
