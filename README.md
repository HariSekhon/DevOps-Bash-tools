...
- `mp3_set_track_order.sh` - sets the track order metadata for mp3 files under given directories to follow the lexical file naming order. Useful for correctly ordering albums songs and audiobook chapters eg. for Mac's Books.app. Especially useful for enforcing global ordering on multiple CD audiobooks after grouping them into a single audiobook using `mp3_set_album.sh` (otherwise they'd interleave in Mac's Books.app eg. this prevents CD1 track1, CD2 track1, CD1 track2 ...)
