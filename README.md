...
- `git/*.sh` - [Git](https://git-scm.com/) scripts:
  - `precommit_run_changed_files.sh` - runs pre-commit on all files changes on the current branch vs the default branch. Useful to reproduce `pre-commit` checks that will be failing in pull requests to get your PRs to pass
