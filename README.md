...
- `gcp_secret_manager_load_kubernetes_combined.sh` - creates a Kubernetes secret from multiple Google Secret Manager secrets (used to put `private.pem` and `public.pem` into the same secret to appear on disk mounts for apps to use)
