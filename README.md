...
- [AWS](https://aws.amazon.com/) scripts - `aws_*.sh`:
  ...
  - `aws_generate_profile_direnvs.sh` - generates subdirectories containing the `config.ini` for every AWS profile found in the given file or `$AWS_CONFIG_FILE` or `~/.aws/config`. Useful to take a large generated AWS `config.ini` from `aws_sso_configs.sh` and then split it into subdirectories for direnvs
