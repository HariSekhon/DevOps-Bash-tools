...
- `terraform_cloud_*.sh` - [Terraform Cloud](https://www.terraform.io/cloud) API scripts:
  ...
  - `terraform_import_github_teams.sh` - finds all github_teams in `./*.tf' code that are not in Terraform state, then queries the GitHub API for their IDs and imports them. See also `github_teams_not_in_terraform.sh`
