...
- `git/*.sh` - [Git](https://git-scm.com/) scripts:
  ...
  - `git_file_uncommitted_changes.sh` - returns zero if given file(s) don't have uncommitted changes to Git, either staged or unstaged. Useful to be able to iterate over git files with in-place edits only if safe to do so without other uncommitted changes that would be at risk of being lost
